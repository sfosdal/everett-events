// Everett Events — calendar UI over events.json.
(function () {
  'use strict';
  ...
  var VENUE_VARS = {
    'Angel of the Winds Arena': '--v-aotw',
    'Funko Field': '--v-funko',
    'APEX Everett': '--v-apex',
    'Port of Everett': '--v-port',
  };
  ...
  // Venue list order = usefulness to a downtown Everett bar owner: the big
  // crowd-drivers first. Anything unexpected slots in after the named
  // venues, busiest first.
  var VENUE_RANK = {
    'Angel of the Winds Arena': 0, // 8k on hockey nights, right downtown
    'Funko Field': 1,              // summer ball a mile up Broadway
    'APEX Everett': 2,             // downtown music hall
    'Port of Everett': 3,          // waterfront festivals and markets
  };
  ...
  // Local teams — an exclusion filter ("hide Silvertips games"), matched on
  // the title no matter what the venue. Mirrors TEAMS in scripts/badges.mjs.
  var TEAMS = [
    { slug: 'silvertips', label: 'Silvertips', re: /silvertips/i },
    { slug: 'aquasox', label: 'AquaSox', re: /aquasox/i },
    { slug: 'wolfpack', label: 'Wolfpack', re: /wolfpack/i },
  ];
  var TEAM_BY_SLUG = {};
  ...
  var PRESET_VENUES = ['Angel of the Winds Arena', 'Funko Field', 'APEX Everett', 'Port of Everett'];
  ...
  function saveFilters() {
    try {
      localStorage.setItem('evt-filters', JSON.stringify({ venues: activeVenues(), badges: activeBadges(), teams: activeTeams() }));
    } catch (e) { /* private mode etc. — filters just won't persist */ }
  }
  function loadFilters() {
    try {
      var s = JSON.parse(localStorage.getItem('evt-filters') || '{}');
  ...
  function updateSubscribe() {
    ...
    $('outlookLink').href = 'https://outlook.live.com/calendar/0/addfromweb?url=' +
      encodeURIComponent(icsHref) + '&name=' + encodeURIComponent('Everett Events');
  ...
  // A fresh marquee line every visit ("On deck in Everett" stays as the
  // no-JS fallback in the HTML).
  var TITLES = [
    'Puck drop in Everett',
    'First pitch in Everett',
    'On the glass in Everett',
    'Dock lights on in Everett',
    'Jet city nights in Everett',
    'Salt air and stage lights in Everett',
    'Turn it up, Everett',
    'Full house in Everett',
    'Game on in Everett',
    'Waterfront nights in Everett',
    'Loud and live in Everett',
    'Big nights on Broadway, Everett',
  ];
